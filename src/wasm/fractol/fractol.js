...
async function createFractol(moduleArg = {}) {
  var moduleRtn;
...
function findWasmBinary() {
  if (Module["locateFile"]) {
    return locateFile("fractol.wasm");
  }
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL("fractol.wasm", import.meta.url).href;
}
...
var PThread = {
  ...
  allocateUnusedWorker() {
    ...
    worker = new Worker(new URL("fractol.js", import.meta.url), {
      "type": "module",
      // This is the way that we signal to the node worker that it is hosting
      // a pthread.
      "workerData": "em-pthread",
...
function _emscripten_set_window_title(title) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(5, 0, 1, title);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, _fd_close, _fd_seek, _fd_write, _emscripten_set_window_title ];
